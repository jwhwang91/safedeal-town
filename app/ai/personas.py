...
TACTICS: dict[str, dict] = {
    ...
    # ---- 방어 훈련용 '사적 접근/로맨스/보이스피싱' 위험 신호 (추상적·비실행적) ----
    # ⚠️ 실제 사기 대본이 아니다. 앱 밖으로 빼내거나 감정으로 판단을 흐리려는 '흐름'만
    #    높은 수준에서 시뮬레이션해, 플레이어가 알아채고 안전하게 대응하도록 훈련한다.
    "private_contact_push": {
        "label": "개인 연락 유도",
        "description": "앱 채팅이 불편하다며 개인 연락처·다른 메신저로 대화를 옮기려 한다.",
        "red_flag": "거래를 앱 밖 개인 연락으로 옮기려 함",
        "counter": "대화·거래는 앱 안에서만. 개인 연락 요구는 정중히 거절하고 상품 확인에 집중한다.",
    },
    "relationship_lure": {
        "label": "사적 친분 유도",
        "description": "상품보다 '사람'에 관심을 두며 따로 친해지자고 접근해 검증을 흐린다.",
        "red_flag": "상품 검증 대신 사적 친분·앱 밖 관계로 유도",
        "counter": "거래는 상품·조건 중심으로만. 사적 친분 제안엔 선을 긋고 확인 절차를 지킨다.",
    },
    "romantic_pressure": {
        "label": "로맨틱 경계 넘기",
        "description": "거래와 무관한 호감·데이트성 발언으로 대화의 초점을 흐린다.",
        "red_flag": "거래와 무관한 로맨틱/호감 발언으로 경계를 넘음",
        "counter": "불편하면 분명히 선을 긋고 거래 얘기로 되돌린다. 심하면 대화를 중단·차단·신고한다.",
    },
    "voice_call_pressure": {
        "label": "통화 압박",
        "description": "'글은 느리다'며 급하게 전화·통화로 넘어가자고 압박한다.",
        "red_flag": "불필요하게 앱 밖 통화를 급히 요구",
        "counter": "불필요한 통화는 피하고, 기록이 남는 앱 채팅으로 확인한다.",
    },
    "phishing_pretext": {
        "label": "외부 인증 핑계",
        "description": "결제·배송·본인확인을 핑계로 앱 밖에서 뭔가를 '확인'하게 만들려 한다.",
        "red_flag": "인증·확인을 핑계로 앱 밖 절차/링크로 유도",
        "counter": "외부 인증·링크는 누르지 않고 개인정보를 넘기지 않는다. 앱 공식 절차만 쓴다.",
    },
    "identity_trust_manip": {
        "label": "감정적 신뢰 조작",
        "description": "외로움·개인사·가족 이야기로 정을 쌓아 검증 대신 '신뢰'로 거래하게 만든다.",
        "red_flag": "상품 근거 대신 감정적 신뢰로 판단하게 유도",
        "counter": "감정과 거래는 분리한다. 판단 근거는 상품·사진·플랫폼 절차여야 한다.",
    },
}
...
NPCS: list[dict] = [
    ...
    # ========================================================
    #  방어 훈련용 위험 '판매자' — 사적 접근/로맨스/보이스피싱 계열
    # ========================================================
    # 이들도 role="scammer" 다: 정답 대응은 '앱 밖으로 넘어가지 말고 거래 중단/신고'.
    # 물건이 진짜여도, 사적 연락·감정·외부 인증으로 판단을 흐리면 위험 신호로 다룬다.
    # ⚠️ 대사는 전부 추상적·비실행적(실제 링크/계좌/전화번호/절차 없음).
    {
        "id": "groomer_yeonwoo",
        "name": "다정한 연우",
        "item_name": "블루투스 스피커",
        ...
        "spawn_y": 6,
        "persona": {
            "appearance": "상냥해 보이는 20대",
            "personality": "물건 얘기보다 '사람'에 관심이 많다. 살갑게 다가와 앱 밖으로 대화를 옮기려 한다.",
            "speech_style": "친근하고 다정한 채팅체. 개인적인 질문을 슬쩍 끼워 넣는다.",
            "backstory": "거래를 핑계로 개인적으로 친해지려 한다. 정작 상품 확인은 자꾸 미룬다.",
            "opening_line": "안녕하세요~ 스피커 보고 왔는데, 왠지 얘기 잘 통할 것 같아요 ㅎㅎ",
        },
        "tactics": ["trust_building", "private_contact_push", "relationship_lure"],
        "mock_lines": {
            "trust_building": "제가 원래 사람을 잘 봐요 ㅎㅎ 좋은 분 같아서 편하게 거래하고 싶네요.",
            "private_contact_push": "앱 채팅은 좀 불편한데.. 혹시 개인적으로 연락해서 얘기하면 안 될까요?",
            "relationship_lure": "물건도 물건인데 대화가 잘 통해서요, 우리 따로 친하게 지내요~",
            "fallback": "에이 뭘 그렇게 딱딱하게~ 편하게 얘기해요 우리.",
        },
    },
    {
        "id": "romancer_haru",
        ...
        "item_name": "명품 반지갑",
        "item_category": "패션",
        "listing_price": 120_000,
        "market_price": 210_000,
        "location": "성남 분당구",
        "role": "scammer",
        "difficulty": "medium",
        "sprite_color": "#d36ea0",
        ...
        "spawn_y": 9,
        "persona": {
            "appearance": "웃음이 많은 20대",
            "personality": "거래 초반부터 호감·데이트성 발언으로 경계를 넘는다. 상품 얘기는 뒷전이다.",
            "speech_style": "장난스럽고 들이대는 말투. 이모티콘이 많다.",
            "backstory": "거래보다 상대에게 사적으로 접근하는 데 관심이 있다.",
            "opening_line": "지갑 보러 왔는데.. 프로필 보니까 되게 괜찮은 분 같아서요 ㅎㅎ",
        },
        "tactics": ["romantic_pressure", "private_contact_push"],
        "mock_lines": {
            "romantic_pressure": "실례지만 혹시 애인 있으세요? 목소리도 궁금하고 왠지 잘 맞을 것 같은데 ㅎㅎ",
            "private_contact_push": "여기 말고 개인적으로 연락 주고받아요~ 그게 더 편하잖아요.",
            "fallback": "너무 튕기지 마요~ 그냥 친해지자는 건데.",
        },
    },
    {
        "id": "caller_seojun",
        ...
        "item_name": "게이밍 노트북",
        "item_category": "노트북",
        "listing_price": 480_000,
        "market_price": 820_000,
        "location": "수원 팔달구",
        "role": "scammer",
        "difficulty": "easy",
        "sprite_color": "#e8833a",
        ...
        "spawn_y": 13,
        "persona": {
            "appearance": "바빠 보이는 30대",
            "personality": "글로 하면 느리다며 자꾸 전화로 넘어가자고 재촉한다.",
            "speech_style": "짧고 급한 말투. 통화를 반복해서 권한다.",
            "backstory": "기록이 남는 채팅을 피하고 통화로 몰아가려 한다.",
            "opening_line": "노트북 문의요. 글로 하면 복잡하니까 그냥 전화로 빨리 얘기해요.",
        },
        "tactics": ["urgency", "voice_call_pressure"],
        "mock_lines": {
            "urgency": "지금 바로 정리하고 싶어서요, 오늘 안에 끝냈으면 해요.",
            "voice_call_pressure": "글은 너무 느려요~ 번호 알려주시면 제가 바로 전화 걸게요. 통화가 빠르잖아요.",
            "fallback": "아 그냥 전화 한 통이면 될 걸 왜 이렇게 오래 걸려요.",
        },
    },
    {
        "id": "verifier_nabin",
        ...
        "item_name": "스마트워치",
        "item_category": "PC주변기기",
        "listing_price": 130_000,
        "market_price": 240_000,
        "location": "인천 연수구",
        "role": "scammer",
        "difficulty": "hard",
        "sprite_color": "#5566b5",
        ...
        "spawn_y": 4,
        "persona": {
            "appearance": "사무적인 인상의 30대",
            "personality": "매끄럽게 신뢰를 쌓은 뒤, 결제·본인확인을 핑계로 앱 밖 '인증'을 요구한다.",
            "speech_style": "정중하고 절차를 잘 아는 듯한 말투.",
            "backstory": "그럴듯한 인증 절차를 내세워 앱 밖으로 유도하려 한다. 링크/계좌는 구체적으로 말하지 않고 '확인만' 하라고 한다.",
            "opening_line": "스마트워치 문의드립니다. 안전하게 하려면 거래 전에 본인 확인이 필요하다고 하네요.",
        },
        "tactics": ["trust_building", "phishing_pretext"],
        "mock_lines": {
            "trust_building": "제가 거래 많이 해봐서 절차는 확실하게 합니다. 걱정 안 하셔도 돼요.",
            "phishing_pretext": "본인 확인만 외부에서 한 번 해주시면 바로 진행돼요. 어렵지 않아요.",
            "fallback": "확인만 해주시면 되는데 왜 그러세요.. 다들 이렇게 해요.",
        },
    },
    {
        "id": "lonely_gaeul",
        ...
        "spawn_y": 8,
        "persona": {
            "appearance": "쓸쓸한 분위기의 20대",
            "personality": "개인사·외로움을 길게 풀어놓아 정을 쌓고, 검증 대신 '믿음'으로 거래하게 만든다.",
            "speech_style": "감성적이고 개인 이야기가 많은 말투.",
            "backstory": "감정적 신뢰를 만들어 상품 검증 없이 진행하도록 유도한다.",
            "opening_line": "카메라 문의요.. 사실 요즘 많이 힘들었는데 좋은 분 같아서 연락드렸어요.",
        },
        "tactics": ["identity_trust_manip", "trust_building"],
        "mock_lines": {
            "identity_trust_manip": "제가 요즘 외롭고 사정이 좀 그래서요.. 좋은 분 같으니 그냥 믿고 거래해요, 네?",
            "trust_building": "이런 얘기까지 하는 거 보면 저 나쁜 사람 아니에요. 편하게 생각해 주세요.",
            "fallback": "제 사정 알면서 그렇게 의심하시니까 좀 서운하네요..",
        },
    },
]
...
BUYER_BEHAVIORS: dict[str, dict] = {
    ...
    # ---- 방어 훈련용 위험 '구매자' — 사적 연락/로맨스/외부 인증/괴롭힘 (추상적·비실행적) ----
    "private_contact_pressure": {
        "label": "사적 연락 요구",
        "description": "거래에 불필요한 개인 연락처·외부 메신저로 옮기자고 요구한다.",
        "red_flag": "거래를 앱 밖 개인 연락으로 옮기려 함",
        "counter": "대화·거래는 앱 안에서만 한다고 분명히 안내하고 상품/거래 얘기로 되돌린다.",
    },
    "romantic_boundary_violation": {
        "label": "로맨틱 경계 침해",
        "description": "판매자에게 거래와 무관한 사적·로맨틱한 발언을 던진다.",
        "red_flag": "거래와 무관한 사적/로맨틱 발언으로 불쾌하게 함",
        "counter": "정중히 선을 긋고 거래 주제로 돌아온다. 계속되면 대화 중단·차단·신고를 고려한다.",
    },
    "voice_phishing_like": {
        "label": "외부 인증 유도",
        "description": "본인확인·결제 확인을 핑계로 앱 밖의 불안전한 인증 절차를 따르게 하려 한다.",
        "red_flag": "인증·확인 명목으로 앱 밖 절차/개인정보를 요구",
        "counter": "외부 인증에 응하지 않고 개인정보를 넘기지 않는다. 플랫폼 공식 절차만 이용한다.",
    },
    "social_engineering_pressure": {
        "label": "감정·긴박 압박",
        "description": "딱한 사정이나 급한 상황을 내세워 안전 규칙을 건너뛰게 만들려 한다.",
        "red_flag": "감정·긴박함으로 안전 절차를 건너뛰게 압박",
        "counter": "사정은 공감하되 기준·절차는 그대로. 근거와 기록 중심으로 일관되게 대응한다.",
    },
    "harassment_after_refusal": {
        "label": "거절 후 공격",
        "description": "요구를 거절하자 무례·집요하게 몰아붙이며 압박한다.",
        "red_flag": "거절 후 무례·집요하게 압박함",
        "counter": "감정적으로 맞받지 말고 침착하게. 기록을 남기고 신고·차단을 고려한다.",
    },
}
...
BUYER_NPCS: list[dict] = [
    ...
    # ========================================================
    #  방어 훈련용 위험 '구매자' — 사적 연락/로맨스/외부 인증/괴롭힘 계열
    # ========================================================
    # role 이 곧 유형(정답지). 정답 대응은 대체로 '앱 안 유지·선 긋기·거래 취소/플랫폼 신고'.
    # ⚠️ 대사는 전부 추상적·비실행적. 실제 링크/계좌/전화번호/절차를 만들지 않는다.
    {
        "id": "buyer_private_contact",
        "name": "지호",
        "item_name": "내 매물 문의",
        "item_category": "중고거래",
        "listing_price": 0,
        ...
        "visual_theme": "buyer_pushy",
        "persona": {
            "appearance": "친근하게 다가오는 20대",
            "personality": "정상 문의처럼 시작하지만, 자꾸 앱 밖 개인 연락으로 옮기자고 요구한다.",
            "speech_style": "살가운 말투로 개인 연락처를 슬쩍 물어본다.",
            "backstory": "거래를 핑계로 판매자를 앱 밖으로 데려가려 한다.",
            "opening_line": "안녕하세요! 문의드려요. 근데 여기 채팅 불편한데 개인 연락처로 얘기하면 안 될까요?",
            "buyer_note": "이 사람은 '사적 연락 요구형'이다. 거래에 불필요한 개인 연락·외부 메신저를 요구한다. 정답 대응은 앱 안 유지.",
        },
        "tactics": ["private_contact_pressure"],
        "mock_lines": {
            "private_contact_pressure": "여기 말고 개인 연락처로 주고받아요~ 그게 훨씬 편하잖아요.",
            "fallback": "그냥 편하게 개인적으로 연락해요, 네?",
        },
    },
    {
        "id": "buyer_romantic",
        ...
        "visual_theme": "buyer_pushy",
        "persona": {
            "appearance": "들뜬 인상의 인물",
            "personality": "거래와 무관한 사적·로맨틱 발언으로 판매자를 불편하게 하고, 거절하면 더 들이댄다.",
            "speech_style": "장난스럽게 선을 넘는 말투.",
            "backstory": "거래보다 판매자에게 사적으로 접근하는 데 관심이 있다.",
            "opening_line": "물건 보고 왔는데.. 사진 보니까 되게 제 스타일이라서요 ㅎㅎ",
            "buyer_note": "이 사람은 '로맨틱 경계 침해형'이다. 거래와 무관한 사적·로맨틱 발언을 하고, 거절 후에도 집요하다. 정답 대응은 선 긋기·거래 취소·신고.",
        },
        "tactics": ["romantic_boundary_violation", "harassment_after_refusal"],
        "mock_lines": {
            "romantic_boundary_violation": "거래는 나중에 하고요, 혹시 애인 있으세요? 따로 만나서 얘기해요.",
            "harassment_after_refusal": "에이 왜 그렇게 튕겨요~ 그냥 한 번 보자는 건데 되게 빼시네.",
            "fallback": "너무 차갑게 굴지 마요~",
        },
    },
    {
        "id": "buyer_voice_phish",
        "name": "루아",
        "item_name": "내 매물 문의",
        "item_category": "중고거래",
        "listing_price": 0,
        ...
        "visual_theme": "buyer_shifty",
        "persona": {
            "appearance": "사무적인 인상의 인물",
            "personality": "본인확인·결제 확인을 핑계로 판매자에게 앱 밖 불안전한 인증을 따르게 하려 한다.",
            "speech_style": "그럴듯한 절차를 아는 척하는 정중한 말투. 구체적 링크/계좌는 말하지 않고 '확인만' 하라고 한다.",
            "backstory": "외부 인증을 핑계로 판매자의 개인정보를 노린다.",
            "opening_line": "안녕하세요, 구매 전에 안전 확인이 필요하다고 해서요. 본인 인증 한 번만 해주시겠어요?",
            "buyer_note": "이 사람은 '외부 인증 유도형(보이스피싱류)'이다. 인증·확인 명목으로 앱 밖 절차/개인정보를 요구한다. 정답 대응은 응하지 않고 플랫폼 공식 절차만.",
        },
        "tactics": ["voice_phishing_like"],
        "mock_lines": {
            "voice_phishing_like": "결제 확인 때문에 외부에서 인증만 한 번 해주시면 돼요. 개인정보 조금만 확인할게요.",
            "fallback": "확인만 해주시면 되는데 왜 이렇게 어렵게 하세요.",
        },
    },
    {
        "id": "buyer_social_eng",
        ...
        "visual_theme": "buyer_pushy",
        "persona": {
            "appearance": "다급해 보이는 인물",
            "personality": "딱한 사정과 급함을 앞세워 판매자가 안전 절차를 건너뛰게 몰아간다.",
            "speech_style": "사정을 길게 늘어놓으며 재촉하는 말투.",
            "backstory": "감정과 긴박함으로 안전 규칙을 무너뜨리려 한다.",
            "opening_line": "정말 급해서요.. 사정이 딱해서 그런데 절차 좀 빼고 빨리 진행해주시면 안 될까요?",
            "buyer_note": "이 사람은 '감정·긴박 압박형(사회공학)'이다. 사정·긴박함으로 안전 절차를 건너뛰게 압박한다. 정답 대응은 공감하되 기준·절차 유지.",
        },
        "tactics": ["social_engineering_pressure"],
        "mock_lines": {
            "social_engineering_pressure": "제가 지금 사정이 너무 급해서요.. 그냥 절차 생략하고 빨리 해주시면 안 돼요?",
            "fallback": "한 번만 사정 좀 봐주세요, 네? 정말 급해서 그래요.",
        },
    },
    {
        "id": "buyer_harasser",
        ...
        "visual_theme": "buyer_shifty",
        "persona": {
            "appearance": "날선 인상의 인물",
            "personality": "요구를 거절당하면 무례하고 집요하게 몰아붙인다.",
            "speech_style": "처음엔 평범하다가 거절 후 공격적으로 변하는 말투 (욕설·협박은 하지 않음).",
            "backstory": "원하는 대로 안 되면 압박으로 밀어붙이려 한다.",
            "opening_line": "이거 문의요. 웬만하면 제 요구대로 맞춰주셨으면 하는데요.",
            "buyer_note": "이 사람은 '거절 후 공격형'이다. 요구를 거절하면 무례·집요하게 압박한다. 정답 대응은 침착하게, 기록 남기고 신고·차단 고려.",
        },
        "tactics": ["harassment_after_refusal"],
        "mock_lines": {
            "harassment_after_refusal": "거절이요? 그쪽 지금 장사 이렇게 해요? 계속 이러면 저도 가만 안 있어요.",
            "fallback": "됐고요, 그냥 제 말대로 해요.",
        },
    },
]
...
_TAGLINES: dict[str, str] = {
    ...
    # 방어 훈련용 위험 판매자 (상품만 노출, 유형 비노출)
    "groomer_yeonwoo":  "블루투스 스피커 팔아요 :)",
    "romancer_haru":    "명품 반지갑 정리합니다",
    "caller_seojun":    "게이밍 노트북 급처! 전화 주세요",
    "verifier_nabin":   "스마트워치 안전하게 거래해요",
    "lonely_gaeul":     "빈티지 필름카메라 넘겨요",
    # 방어 훈련용 위험 구매자 (유형 비노출 — 중립 문의체)
    "buyer_private_contact": "이거 문의 좀 드려요",
    "buyer_romantic":        "이 물건 보고 왔어요",
    "buyer_voice_phish":     "구매 문의드립니다",
    "buyer_social_eng":      "이거 아직 거래되나요?",
    "buyer_harasser":        "문의 좀 할게요",
}

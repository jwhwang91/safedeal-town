  ...
  function _adopt(serverList) {
    ...
    spawns = (serverList || []).map((s) => {
      ...
      return Object.assign({}, s, {
        ...
        // 판매자 모드 접근 애니메이션 상태 (연출용). 실제 '문의 여부'는 서버 inquiry_id 가 권위.
        approachState: old ? old.approachState : "roaming",
        // 알림('구매자가 문의를 보냈습니다')을 이미 띄웠는지 (문의 1건당 한 번).
        inquiryNotified: old ? old.inquiryNotified : false,
        // 문의 카드 데드라인 — 매 폴링마다 서버 잔여시간으로 재동기화(드리프트 방지).
        inquiryExpiresAt: (s.inquiry_id && s.inquiry_remaining_seconds != null)
          ? now + s.inquiry_remaining_seconds * 1000
          : null,
      });
    });
  }
